...
if (grepl("LGA-LYP6123", computer)) {
    ...
    results_dirpath = "/media/lheraut/Explore2/hydrological-projections_TRACC-indicators/hydrological-projections_TRACC-indicators_yearly-variables_by-chain_fst"
...
Results_path = list.files(results_dirpath, recursive=TRUE,
                          full.names=TRUE)
Chain_dirpath = gsub(".fst", "", basename(Results_path))
...
# Chain_dirpath_list =
    # lapply(climateChain_dirpath,
           # function (x) Chain_dirpath[climateChain_dirpath_ALL == x])
...
nClimateChain_dirpath = length(ClimateChain_dirpath)

if (MPI == "file") {            
    start = ceiling(seq(1, nClimateChain_dirpath,
                        by=(nClimateChain_dirpath/size)))
    if (any(diff(start) == 0)) {
        start = 1:nClimateChain_dirpath
        end = start
    } else {
        end = c(start[-1]-1, nClimateChain_dirpath)
    }
    if (rank == 0) {
    ...
    if (Rrank+1 > nClimateChain_dirpath) {
        ClimateChain_dirpath = NULL
    } else {
        ClimateChain_dirpath = ClimateChain_dirpath[start[Rrank+1]:end[Rrank+1]]
    }
}
...
for (i in 1:nClimateChain_dirpath) {
    if (nClimateChain_dirpath == 0) {
        break
    }
    climatechain_dirpath = ClimateChain_dirpath[i]

    ####
    # chain_dirpath = "rcp85_MPI-ESM-LR_REMO_ADAMONT_CTRIP"
    ###
    
    is_SAFRAN = grepl("SAFRAN", basename(climatechain_dirpath))
    
    post(paste0("* ", i, " -> ",
                round(i/nClimateChain_dirpath*100, 1), "%"))
    post(paste0(climatechain_dirpath, collapse=" / "))
    
    Projection_chain =
        dplyr::filter(Projection,
                      grepl(climatechain_dirpath,
                            gsub("historical-", "",
                                 Projection$Chain)))
    ...
    for (j in 1:nWL) {
        ...
        Ok = grepl(Variable_pattern, Var_ALL)
        ...
        for (k in 1:nVar_path_list) {
            ...
            if (duration < 20) {
                ...
                dataEX = tidyr::fill(dplyr::group_by(dataEX, code),
                                     GWL, Chain, EXP, GCM, RCM,
                                     BC, .direction="downup") #HM
