...
HM_ALL_paths =sapply(basename(data_paths),
       function(x) paste0(unlist(strsplit(x, "_"))[c(2, 9)],
                          collapse=".*"), USE.NAMES=FALSE)
HM_paths = gsub("[-]", "[-]", unique(HM_ALL_paths))

data_paths_HM_selection = c()
for (x in HM_paths) {
    data_paths_HM_selection =
        c(data_paths_HM_selection,
          data_paths[grepl(x, data_paths)][1])
}

for (v in V) {
    assign(paste0(v, "_data"), dplyr::tibble())
}

NCx_names = c()
for (x in data_paths_HM_selection) {
    NCx = ncdf4::nc_open(x)
    NCx_names = c(NCx_names, names(NCx$var))
    Code_nc = ncdf4::ncvar_get(NCx, "code")

    for (v in V) {
        if (!any(get(v) %in% names(NCx$var))) {
            next
        }
        ncvar = get(v)[get(v) %in% names(NCx$var)]
        ncdata = ncdf4::ncvar_get(NCx, ncvar)
        v_data = paste0(v, "_data")
        assign(v_data,
               dplyr::bind_rows(get(v_data),
                                dplyr::tibble(code=Code_nc,
                                              value=ncdata)))
    }
    ncdf4::nc_close(NCx)
}

NCx_names = unique(NCx_names)

for (v in V) {
    if (!any(get(v) %in% NCx_names)) {
        next
    }
    v_data = paste0(v, "_data")
    tmp = dplyr::summarise(dplyr::group_by(get(v_data),
                                           code),
                           value=mean(value, na.rm=TRUE),
                           .groups="drop")
    tmp = tidyr::complete(tmp, code=Code)
    idCode_match = match(Code, tmp$code)
    idCode_match = idCode_match[!is.na(idCode_match)]
    assign(v_data, tmp$value[idCode_match])
}
...
if (any(WGS84_lon_model %in% NCx_names)) {
...
if (any(WGS84_lat_model %in% NCx_names)) {
...
if (any(L93_X_model %in% NCx_names)) {
...
if (any(L93_Y_model %in% NCx_names)) {
...
if (any(LII_X_model %in% NCx_names)) {
...
if (any(LII_Y_model %in% NCx_names)) {
